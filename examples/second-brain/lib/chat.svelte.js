	...
	try {
		const result = await app.ask(q, {
			...
			on_token: (_delta, full) => {
				chat.messages[at].content = full;
			}
		});
		Object.assign(chat.messages[at], { content: result.answer, sources: result.sources, cited: result.cited, streaming: false });
	} catch (err) {
		Object.assign(chat.messages[at], { streaming: false, error: err.name === 'AbortError' ? 'stopped' : err.message });
	} finally {
