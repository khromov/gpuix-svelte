...
/** @typedef {{ root: string, db: string, files: string, thumbs: string, models: string, tmp: string }} DataDirs */
 ...
 * @returns {DataDirs}
 */
export function data_dirs(root = process.env.GPUIX_BRAIN_DIR || default_root()) {
	const dirs = {
		root,
...
export const file_path = (dirs, id, ext) => join(dirs.files, `${id}.${String(ext).replace(/^\./, '').toLowerCase()}`);
export const thumb_path = (dirs, id) => join(dirs.thumbs, `${id}.webp`);
