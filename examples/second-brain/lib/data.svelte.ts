...
/** @type {{ items: any[], progress: Record<number, { step: string | null, progress: number | null, text: string | null }>,
 *   counts: { total: number, by_kind: Record<string, number>, pending: number, error: number },
 *   queue: { pending: number, active: number }, ml: any, capabilities: any, ready: boolean }} */
export const data = $state({
	...
	ml: { embed: { state: 'unloaded' }, whisper: { state: 'unloaded' }, clip: { state: 'unloaded' }, worker: 'down', error: null },
	capabilities: null,
	ready: false
});
...
export const display_title = (item) => item.title || (item.kind === 'link' ? item.source_url : '') || 'Untitled';

export function preview(item, max = 160) {
	let body = item.body ?? '';
...
export const format_bytes = (bytes) => (bytes >= 1073741824 ? `${(bytes / 1073741824).toFixed(1)} GB` : `${Math.round(bytes / 1048576)} MB`);
