...
Only the events GPUI knows can fire: `click`, `mousedown` / `mouseup` / `mousemove`, `mouseenter` / `mouseleave`, `keydown` / `keyup` (these need focus: `tabindex` or `autofocus`), `focus` / `blur`, `scroll`, and `change` / `submit` on inputs. Svelte lowercases the attribute name and the renderer maps it back to GPUI's camelCase (`onmouseenter` → `mouseEnter`); unknown names are dropped silently. App-wide shortcuts skip focus altogether: `on_window_key('keydown', handler)` fires whatever has focus, with `e.editing` telling you a text field is receiving the same key — this tutorial's ← → navigation works that way.
