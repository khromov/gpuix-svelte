...
- **No scrollbars are painted.** The thumbs on both panels here are drawn by `gpuix-svelte/components/Scroller.svelte`: it reads `getElementBounds()` and `getScrollOffset()` through `get_native()` and calls `scrollTo()` while you drag.
