...
- Plain `.js` modules are not busted: `steps.js` and `theme.js` in this tutorial need a restart, while the samples reload live.
...
**Exercise:** open `examples/tutorial/samples/Hello.svelte`, change the greeting, save — and watch the preview on the right. You stay on this step because the tutorial keeps its index in a `globalThis` slot, exactly as `render.js` keeps the window in one.
